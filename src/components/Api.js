export class Api {
  ...
  constructor(options) { // принимает объект options  с двумя свойствами
    this._headers = options.headers; // объект с заголовками, которые будут использоваться в запросах (например, Content-Type или токен авторизации).
    this._serverURL = options.serverURL; // базовый URL сервера, к которому будут отправляться запросы.
    /** возвращает ответ / ошибку после выполнения промиса */
    this._handlePromiseReturn = ((res) => { // проверяет статус ответа
 ...
 //Этот код описывает метод getUserInfo класса, который предназначен для получения данных о текущем пользователе с сервера через API.

 /*Promise (или чаще CompletableFuture, Future и другие) — это объект, представляющий результат асинхронной операции, который может быть доступен сейчас, позже или никогда.*/
  ...
  /*Этот тип указывает, что асинхронная операция, представленная этим промисом, вернет объект Response при успешном выполнении. */

  /*PATCH — это HTTP-метод, используемый для частичного обновления ресурса на сервере. В отличие от метода PUT, который заменяет весь ресурс,
   PATCH позволяет обновлять только отдельные поля или части ресурса, сохраняя остальные без изменений. */
